...
public class AdminRoomInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		while (parameter.hasMoreElements()) {
			...
			// System.out.println(parameter.nextElement().toString());

			String value = parameter.nextElement().toString();
			switch (value) {
			case "PS_Weekday":
				pv.setPsWeekday(Integer.parseInt(multi.getParameter(value)));
				break;
			case "PS_MaxPerson":
				pv.setPsMaxPersonnel(Integer.parseInt(multi.getParameter(value)));
				break;
			case "PS_Name":
				pv.setPsName(multi.getParameter(value));
				break;
			case "PS_Contents":
				pv.setPsContents(multi.getParameter(value));
				break;
			case "PS_Addition_Price":
				pv.setPsAddtionalPrice(Integer.parseInt(multi.getParameter(value)));
				break;
			case "PS_Person":
				pv.setPsPersonnel(Integer.parseInt(multi.getParameter(value)));
				break;
			case "PS_Weekend":
				pv.setPsWeekend(Integer.parseInt(multi.getParameter(value)));
				break;
			}
		}
		...
		while (formNames.hasMoreElements()) {
			...
			if (fileValue.equals("input_main")) {
				yn = "Y";
			} else {
				yn = "N";
			}
			String fileName = multi.getFilesystemName(fileValue); // 파일명
			if (fileName != null) {
				// String type = multi.getContentType(fileValue); //콘텐트타입
				// File fileObj = multi.getFile(fileValue); //파일객체
				// String originFileName = multi.getOriginalFileName(fileValue); //초기 파일명
				// String fileExtend = fileName.substring(fileName.lastIndexOf(".")+1); //파일 확장자
				// String fileSize = String.valueOf(fileObj.length()); // 파일크기
			...
			// File f = multi.getFile(fileValue);
			// System.out.println(f);
			// System.out.println(f);
			// System.out.println(piv.toString());
		}
